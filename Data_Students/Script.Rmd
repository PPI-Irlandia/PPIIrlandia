...
```{r City, echo=FALSE, message=FALSE, warning=FALSE, quiet = TRUE}
...
Ireland %>%
  ...
  tm_layout(frame = FALSE,
            ...
            main.title.size = 1)  # Adjust size as needed
```
...
```{r Major, echo=FALSE, message=FALSE, warning=FALSE}
data_summary <- data %>%
  count(Major_General) %>%
  arrange(n) %>%                    
...
```{r degree, echo=FALSE, message=FALSE, warning=FALSE}
degree_counts <- data %>%
  count(Degree) %>%
  mutate(
    percent = round(100 * n / sum(n), 1),
...
```{r Funding, echo=FALSE, message=FALSE, warning=FALSE}
#### Funding ####
data_summary <- data %>%
  count(Funding) %>%
  arrange(n) %>%                    
